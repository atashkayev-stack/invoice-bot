...
class PDFFromTemplateV2:
    ...
    def generate_invoice_pdf(self, data: dict, profile: dict, with_xml: bool = False) -> io.BytesIO:
        """Генерация PDF из invoice_template_v2.html"""
        template_path = os.path.join(self.templates_dir, "invoice_template_v2.html")
    ...
    def _prepare_invoice_data(self, data: dict, profile: dict) -> dict:
        ...
        if vat_per_item:
            for item in items:
                item_vat = item.get('total_price', 0) * item.get('vat_rate', 0) / 100
                total_vat += item_vat
        ...
        if skonto_percentage > 0 and skonto_days > 0:
            skonto_total = (total_net + total_vat) * (1 - skonto_percentage / 100)
            invoice_date = datetime.strptime(data.get('invoice_date'), '%Y-%m-%d')
            skonto_date = (invoice_date + timedelta(days=skonto_days)).strftime('%d.%m.%Y')
        ...
        return {
            ...
            'sender_vat_id': profile.get('vat_id'),
            'sender_iban': profile.get('iban'),
            'sender_bic': profile.get('bic'),
            'bank_name': profile.get('bank_name'),
            
            # Client
            'client_company': data.get('client_name', ''),
            'client_address': f"{data.get('client_street', '')}, {data.get('client_postal_code', '')} {data.get('client_city', '')}".strip(', '),
            ...
            'delivery_date': self._format_date(data.get('delivery_date')),
            ...
            'total_gross': f"{(total_net + total_vat):.2f}",
            'vat_rate': f"{global_vat:.0f}",
            
            # Skonto
            'skonto_percentage': skonto_percentage,
            'skonto_days': skonto_days,
            'skonto_total': f"{skonto_total:.2f}" if skonto_total > 0 else None,
            'skonto_date': skonto_date,
            
            # Shipping address
            'ship_to_name': data.get('ship_to_name'),
            'ship_to_street': data.get('ship_to_street'),
            'ship_to_postal_code': data.get('ship_to_postal_code'),
