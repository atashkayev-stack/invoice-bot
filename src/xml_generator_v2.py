...
class XMLGeneratorV2:
    ...
    def generate_zugferd_xml(self, data: dict, profile: dict) -> str:
        ...
        etree.SubElement(doc, f"{{{self.ns['ram']}}}TypeCode").text = data.get(
        ...
        from src.handlers_v1 import get_vat_info
        ...
        for item in items:
            ...
            assoc = etree.SubElement(
                li, f"{{{self.ns['ram']}}}AssociatedDocumentLineDocument")
            etree.SubElement(assoc, f"{{{self.ns['ram']}}}LineID").text = str(
                item.get('position', 1))

            prod = etree.SubElement(
                li, f"{{{self.ns['ram']}}}SpecifiedTradeProduct")
            etree.SubElement(prod,
                             f"{{{self.ns['ram']}}}Name").text = item.get(
                                 'description', 'Position')

            agr = etree.SubElement(
                li, f"{{{self.ns['ram']}}}SpecifiedLineTradeAgreement")
            gross_price = etree.SubElement(
                agr, f"{{{self.ns['ram']}}}GrossPriceProductTradePrice")
            etree.SubElement(gross_price, f"{{{self.ns['ram']}}}ChargeAmount"
                             ).text = f"{item.get('unit_price', 0):.2f}"

            deliv = etree.SubElement(
                li, f"{{{self.ns['ram']}}}SpecifiedLineTradeDelivery")
            billed = etree.SubElement(deliv,
                                      f"{{{self.ns['ram']}}}BilledQuantity",
                                      unitCode=self._get_unit_code(
                                          item.get('unit', 'Stk')))
            billed.text = str(item.get('quantity', 1))

            settle = etree.SubElement(
                li, f"{{{self.ns['ram']}}}SpecifiedLineTradeSettlement")

            # VAT per item - ИСПРАВЛЕНО: используем get_vat_info
            item_vat_rate = item.get(
                'vat_rate') if vat_per_item else global_vat
            ...
            if vat_info['reason']:
                etree.SubElement(tax, f"{{{self.ns['ram']}}}ExemptionReason"
                                 ).text = vat_info['reason']

            monet = etree.SubElement(
                settle,
                f"{{{self.ns['ram']}}}SpecifiedTradeSettlementLineMonetarySummation"
            )
            etree.SubElement(monet, f"{{{self.ns['ram']}}}LineTotalAmount"
                             ).text = f"{item.get('total', 0):.2f}"
        ...
        if profile.get('legal_form'):
            legal_org = etree.SubElement(
                seller, f"{{{self.ns['ram']}}}SpecifiedLegalOrganization")
            etree.SubElement(
                legal_org,
                f"{{{self.ns['ram']}}}TradingBusinessName").text = profile.get(
                    'legal_form')
            if profile.get('trade_register_number'):
                etree.SubElement(legal_org,
                                 f"{{{self.ns['ram']}}}ID").text = profile.get(
                                     'trade_register_number')

        seller_addr = etree.SubElement(
            seller, f"{{{self.ns['ram']}}}PostalTradeAddress")
        etree.SubElement(
            seller_addr,
            f"{{{self.ns['ram']}}}PostcodeCode").text = profile.get(
                'postal_code', '')
        etree.SubElement(seller_addr,
                         f"{{{self.ns['ram']}}}LineOne").text = profile.get(
                             'street', '')
        etree.SubElement(seller_addr,
                         f"{{{self.ns['ram']}}}CityName").text = profile.get(
                             'city', '')
        etree.SubElement(seller_addr,
                         f"{{{self.ns['ram']}}}CountryID").text = profile.get(
                             'country_code', 'DE')

        if profile.get('vat_id'):
            seller_tax = etree.SubElement(
                seller, f"{{{self.ns['ram']}}}SpecifiedTaxRegistration")
            etree.SubElement(seller_tax,
                             f"{{{self.ns['ram']}}}ID",
                             schemeID="VA").text = profile.get('vat_id')

        if profile.get('tax_id'):
            seller_tax2 = etree.SubElement(
                seller, f"{{{self.ns['ram']}}}SpecifiedTaxRegistration")
            etree.SubElement(seller_tax2,
                             f"{{{self.ns['ram']}}}ID",
                             schemeID="FC").text = profile.get('tax_id')
        ...
        if data.get('client_type') == 'b2b':
            if data.get('client_legal_form'):
                buyer_legal = etree.SubElement(
                    buyer, f"{{{self.ns['ram']}}}SpecifiedLegalOrganization")
                etree.SubElement(buyer_legal,
                                 f"{{{self.ns['ram']}}}TradingBusinessName"
                                 ).text = data.get('client_legal_form')
                if data.get('client_trade_register'):
                    etree.SubElement(
                        buyer_legal,
                        f"{{{self.ns['ram']}}}ID").text = data.get(
                            'client_trade_register')

        buyer_addr = etree.SubElement(
            buyer, f"{{{self.ns['ram']}}}PostalTradeAddress")
        etree.SubElement(buyer_addr,
                         f"{{{self.ns['ram']}}}PostcodeCode").text = data.get(
                             'client_postal_code', '')
        etree.SubElement(buyer_addr,
                         f"{{{self.ns['ram']}}}LineOne").text = data.get(
                             'client_street', '')
        etree.SubElement(buyer_addr,
                         f"{{{self.ns['ram']}}}CityName").text = data.get(
                             'client_city', '')
        etree.SubElement(buyer_addr,
                         f"{{{self.ns['ram']}}}CountryID").text = data.get(
                             'client_country', 'DE')

        if data.get('client_vat_id'):
            buyer_tax = etree.SubElement(
                buyer, f"{{{self.ns['ram']}}}SpecifiedTaxRegistration")
            etree.SubElement(buyer_tax,
                             f"{{{self.ns['ram']}}}ID",
                             schemeID="VA").text = data.get('client_vat_id')

        # Buyer Reference (ОБЯЗАТЕЛЬНО для B2B)
        if data.get('buyer_reference'):
            etree.SubElement(
                agr, f"{{{self.ns['ram']}}}BuyerReference").text = data.get(
                    'buyer_reference')

        # Purchase Order
        if data.get('purchase_order'):
            buyer_order = etree.SubElement(
                agr, f"{{{self.ns['ram']}}}BuyerOrderReferencedDocument")
            etree.SubElement(
                buyer_order,
                f"{{{self.ns['ram']}}}IssuerAssignedID").text = data.get(
                    'purchase_order')

        # Trade Delivery - КРИТИЧНО: Leistungsdatum обязателен!
        delivery = etree.SubElement(
            tx, f"{{{self.ns['rsm']}}}ApplicableHeaderTradeDelivery")
        deliv_event = etree.SubElement(
            delivery, f"{{{self.ns['ram']}}}ActualDeliverySupplyChainEvent")
        deliv_dt = etree.SubElement(deliv_event,
                                    f"{{{self.ns['ram']}}}OccurrenceDateTime")
        delivery_date = data.get('delivery_date') or data.get(
            'invoice_date') or datetime.now().strftime('%Y%m%d')
        etree.SubElement(deliv_dt,
                         f"{{{self.ns['udt']}}}DateTimeString",
                         format="102").text = delivery_date.replace('-', '')
        ...
        etree.SubElement(
            settle,
            f"{{{self.ns['ram']}}}InvoiceCurrencyCode").text = data.get(
                'currency', 'EUR')
        ...
        etree.SubElement(payment, f"{{{self.ns['ram']}}}TypeCode").text = str(
            data.get('payment_means', '58'))

        if data.get('payment_means') == '58':  # SEPA
            payee = etree.SubElement(
                payment,
                f"{{{self.ns['ram']}}}PayeePartyCreditorFinancialAccount")
            etree.SubElement(payee,
                             f"{{{self.ns['ram']}}}IBANID").text = profile.get(
                                 'iban', '')
            if profile.get('bic'):
                inst = etree.SubElement(
                    payment,
                    f"{{{self.ns['ram']}}}PayeeSpecifiedCreditorFinancialInstitution"
                )
                etree.SubElement(
                    inst,
                    f"{{{self.ns['ram']}}}BICID").text = profile.get('bic')

        # Payment Terms
        terms = etree.SubElement(
            settle, f"{{{self.ns['ram']}}}SpecifiedTradePaymentTerms")
        if data.get('due_date'):
            due = etree.SubElement(terms,
                                   f"{{{self.ns['ram']}}}DueDateDateTime")
            etree.SubElement(due,
                             f"{{{self.ns['udt']}}}DateTimeString",
                             format="102").text = data.get('due_date').replace(
                                 '-', '')

        # Tax Total - ИСПРАВЛЕНО: используем get_vat_info
        global_vat_info = get_vat_info(
            profile, global_vat if global_vat is not None else 19)

        tax_total = etree.SubElement(
            settle, f"{{{self.ns['ram']}}}ApplicableTradeTax")
        etree.SubElement(tax_total, f"{{{self.ns['ram']}}}CalculatedAmount"
                         ).text = f"{data.get('total_vat', 0):.2f}"
        etree.SubElement(tax_total,
                         f"{{{self.ns['ram']}}}TypeCode").text = "VAT"
        etree.SubElement(tax_total, f"{{{self.ns['ram']}}}BasisAmount"
                         ).text = f"{data.get('total_net', 0):.2f}"
        etree.SubElement(tax_total, f"{{{self.ns['ram']}}}CategoryCode"
                         ).text = global_vat_info['category']
        etree.SubElement(tax_total,
                         f"{{{self.ns['ram']}}}RateApplicablePercent"
                         ).text = f"{global_vat_info['rate']:.2f}"

        if global_vat_info['reason']:
            etree.SubElement(tax_total, f"{{{self.ns['ram']}}}ExemptionReason"
                             ).text = global_vat_info['reason']
        ...
        etree.SubElement(monet, f"{{{self.ns['ram']}}}TaxBasisTotalAmount"
                         ).text = f"{data.get('total_net', 0):.2f}"
        etree.SubElement(monet,
                         f"{{{self.ns['ram']}}}TaxTotalAmount",
                         currencyID=data.get(
                             'currency',
                             'EUR')).text = f"{data.get('total_vat', 0):.2f}"
        etree.SubElement(monet, f"{{{self.ns['ram']}}}GrandTotalAmount"
                         ).text = f"{data.get('total_gross', 0):.2f}"
        etree.SubElement(monet, f"{{{self.ns['ram']}}}DuePayableAmount"
                         ).text = f"{data.get('total_gross', 0):.2f}"
