...
class Database:
    ...
    def get_profile(self, user_id: int) -> Optional[Dict]:
        try:
            r = self.client.table("profiles").select("*").eq("id", user_id).execute()
    ...
    def create_profile(self, user_id: int, owner_name: str, username: str = None) -> bool:
        try:
            self.client.table("profiles").insert({"id": user_id, "owner_name": owner_name, "username": username, "invoice_number_prefix": "RE-", "invoice_number_format": 4, "customer_id_prefix": "KUND-", "offer_number_prefix": "ANG-", "offer_number_format": 4}).execute()
            return True
        except Exception as e:
    ...
    def get_all_clients(self, user_id: int) -> List[Dict]:
        try:
            r = self.client.table("clients").select("*").eq("user_id", user_id).order("company_name").execute()
            return r.data or []
        except: return []
    
    def get_client(self, client_id: str) -> Optional[Dict]:
        try:
            r = self.client.table("clients").select("*").eq("id", client_id).execute()
            return r.data[0] if r.data else None
        except: return None
    ...
    def search_clients(self, user_id: int, search: str) -> List[Dict]:
        ...
        s = search.lower()
        return [c for c in all_clients if s in c.get('company_name','').lower() or s in c.get('city','').lower() or s in c.get('customer_id','').lower()]
    
    def create_or_update_client(self, user_id: int, client_data: Dict) -> Optional[str]:
        try:
            if client_data.get('customer_id'):
                existing = self.client.table("clients").select("*").eq("user_id", user_id).eq("customer_id", client_data['customer_id']).execute()
                if existing.data:
                    cid = existing.data[0]['id']
                    self.client.table("clients").update(client_data).eq("id", cid).execute()
                    return cid
    ...
    def get_invoices(self, user_id: int, limit: int = 10) -> List[Dict]:
        try:
            r = self.client.table("invoices").select("*").eq("user_id", user_id).order("created_at", desc=True).limit(limit).execute()
            return r.data or []
        except: return []
    ...
    def update_invoice(self, invoice_id: str, data: Dict) -> bool:
        try:
            self.client.table("invoices").update(data).eq("id", invoice_id).execute()
            return True
        except: return False
    ...
    def create_invoice_items(self, invoice_id: str, items: List[Dict]) -> bool:
        try:
            for idx, item in enumerate(items, 1):
                self.client.table("invoice_items").insert({"invoice_id": invoice_id, "position_number": idx, **item}).execute()
            return True
        except: return False
    
    def get_invoice_items(self, invoice_id: str) -> List[Dict]:
        try:
            r = self.client.table("invoice_items").select("*").eq("invoice_id", invoice_id).order("position_number").execute()
            return r.data or []
        except: return []
    ...
    def get_offers(self, user_id: int, limit: int = 10) -> List[Dict]:
        try:
            r = self.client.table("offers").select("*").eq("user_id", user_id).order("created_at", desc=True).limit(limit).execute()
            return r.data or []
        except: return []
    
    def get_offer(self, offer_id: str) -> Optional[Dict]:
        try:
            r = self.client.table("offers").select("*").eq("id", offer_id).execute()
            return r.data[0] if r.data else None
        except: return None
    
    def update_offer(self, offer_id: str, data: Dict) -> bool:
        try:
            self.client.table("offers").update(data).eq("id", offer_id).execute()
            return True
        except: return False
    ...
    def create_offer_items(self, offer_id: str, items: List[Dict]) -> bool:
        try:
            for idx, item in enumerate(items, 1):
                self.client.table("offer_items").insert({"offer_id": offer_id, "position_number": idx, **item}).execute()
            return True
        except: return False
    
    def get_offer_items(self, offer_id: str) -> List[Dict]:
        try:
            r = self.client.table("offer_items").select("*").eq("offer_id", offer_id).order("position_number").execute()
            return r.data or []
        except: return []
